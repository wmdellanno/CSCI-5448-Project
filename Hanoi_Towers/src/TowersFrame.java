...
public class TowersFrame extends JFrame implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == startBUT) {//begins game
            startGame();
        } else if (e.getSource() == LtoRBUT) {//if move is legal, paints towers after they have been rendered
            if (tm.checkMove(2, 0)) {
        ...
        if (tm.checkWin()) {//checks for win after each move
    ...
    public int[][] renderTowers(int to, int from) {//creates three temp stacks for each tower and returns array with values
        ArrayListStack tempTo = new ArrayListStack();
        ArrayListStack tempFrom = new ArrayListStack();
        ArrayListStack tempOther = new ArrayListStack();
        int other = determineOther(to, from);
        ...
        } while (!tm.towers[other].emptyTower()) { // puts "other" tower on temp stack
            tempOther.push(tm.towers[other].removeDisc());
        }
        ...
        for (int i = 0; !tempOther.isEmpty(); i++) { //renders "other" tower until its empty and puts discs back on orig stack
            temp[other][i] = (Integer) tempOther.peekTop();
            tm.towers[other].addDisc((Integer) tempOther.pop());
        }
        return temp;
    }

    public int determineOther(int to, int from) {
        if (to + from == 3) {
            return 0;
        } else if (to + from == 1) {
            return 2;
